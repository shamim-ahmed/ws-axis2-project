...
public class FetchNewsAction extends ActionSupport implements RequestAware, SessionAware {
  ...
  public String execute() {
	...
	String newsXml = facade.getNews(max, sessionId);

	if (StringUtils.isNotBlank(newsXml)) {
	  String newsHtml = XSLTUtil.transform(newsXml, ContentType.NEWS);
	  requestMap.put("newsHtml", newsHtml);
	}
