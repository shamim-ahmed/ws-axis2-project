...
public class FetchVideosAction extends ActionSupport implements SessionAware, RequestAware {
  ...
  public String execute() {
	...
	String videoXml = facade.getVideos(max, sessionId);
	
	if (StringUtils.isNotBlank(videoXml)) {
	  String videoHtml = XSLTUtil.transform(videoXml, ContentType.VIDEO);
	  requestMap.put("videoHtml", videoHtml);
	}
